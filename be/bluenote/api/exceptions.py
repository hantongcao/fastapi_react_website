...
class bluenoteAPIException(HTTPException):
    pass
...
def http_exception_factory(
    ...
    def init(self, message=default_message, is_bluenote_exception=False):
        if is_bluenote_exception:
            self.__class__.__bases__ = (bluenoteAPIException,)
...
class bluenoteAPIError(BaseModel):
    message: str
...
class bluenoteAPIErrorResponse(BaseModel):
    error: bluenoteAPIError


bluenote_api_error_responses = {
    404: {"model": bluenoteAPIErrorResponse},
    409: {"model": bluenoteAPIErrorResponse},
    401: {"model": bluenoteAPIErrorResponse},
    403: {"model": bluenoteAPIErrorResponse},
    422: {"model": bluenoteAPIErrorResponse},
    400: {"model": bluenoteAPIErrorResponse},
    500: {"model": bluenoteAPIErrorResponse},
    503: {"model": bluenoteAPIErrorResponse},
}
...
def register_handlers(app: FastAPI):
    ...
    @app.exception_handler(bluenoteAPIException)
    async def bluenote_api_exception_handler(request: Request, exc: bluenoteAPIException):
        """
        This handler is used to return error response in bluenote API format.
        """
        return JSONResponse(
            status_code=exc.status_code,
            content={
